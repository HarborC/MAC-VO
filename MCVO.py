...
from DataLoader import SequenceBase, MultiCameraFrame, smart_transform
from Evaluation.EvalSeq import EvaluateSequences
from Odometry.MCVO import MCVO
...
def VisualizeRerunCallback(frame: MultiCameraFrame, system: MCVO, pb: ColoredTqdm):
    ...
    rr_plt.log_image ("/world/macvo/cam_left", frame.data.images[0].permute(1, 2, 0))
...
def VisualizeVRAMUsage(frame: MultiCameraFrame, system: MCVO, pb: ColoredTqdm):
...
def get_args():
    ...
    parser.add_argument("--odom", type=str, default = "Config/Experiment/MCVO/MCVO.yaml")
...
if __name__ == "__main__":
    ...
    def onFrameFinished(frame: MultiCameraFrame, system: MCVO, pb: ColoredTqdm):
        VisualizeRerunCallback(frame, system, pb)
        VisualizeVRAMUsage(frame, system, pb)

    # Initialize data source
    sequence = smart_transform(
        SequenceBase[MultiCameraFrame].instantiate(datacfg.type, datacfg.args).clip(args.seq_from, args.seq_to),
        cfg.Preprocess
    )
    ...
    system = MCVO[MultiCameraFrame].from_config(asNamespace(exp_space.config))
